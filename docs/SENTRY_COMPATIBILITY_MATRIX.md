...
| Transactions | https://develop.sentry.dev/sdk/telemetry/traces/ | `transaction` item | partial | `transaction` items are routed to `worker-transaction`, parsed as Sentry transaction events, and written to ClickHouse `sentry.transactions` plus child `sentry.spans`. | No performance query API, dynamic sampling, trace metrics, span-only ingestion, or transaction/span indexed outcome handling yet. | `testdata/sentry-fixtures/envelopes/transaction.envelope` |
