...
| Rate limits | https://develop.sentry.dev/sdk/expected-features/rate-limiting/ | `X-Sentry-Rate-Limits`, `Retry-After` | partial | Checks rate limits per project key, IP, and envelope category; event category rejection returns `429`; non-event category rejection drops only that category. | Uses one configured limit across all categories; no persisted outcome accounting yet. | Not yet |
