...
| Attachments | https://docs.sentry.io/platforms/javascript/enriching-events/attachments/ | `attachment` item | partial | `attachment` items are fully scanned from envelopes, routed to `worker-attachment`, and stored in Postgres `event_attachments` with filename, content type, attachment type, SHA-1, and bytes. | Uses Postgres bytea instead of object storage; no Sentry-compatible attachment download/list API yet; global envelope size limit is lower than Sentry's documented attachment limits. | `testdata/sentry-fixtures/envelopes/event-with-attachment.envelope` |
