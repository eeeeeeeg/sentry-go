...
| Replays | https://docs.sentry.io/product/session-replay/ | `replay_event`, `replay_recording` items | partial | `replay_event` and `replay_recording` items are routed to `worker-replay` and persisted in Postgres `replay_items` with replay metadata and raw segment payloads. | No Sentry replay list/detail/segment APIs yet; replay payloads are stored in Postgres bytea instead of object storage; segment reassembly semantics are minimal. | `testdata/sentry-fixtures/envelopes/replay.envelope` |
